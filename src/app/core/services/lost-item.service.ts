...
export class LostItemService {
  ...
  getLostItems(): Observable<LostFoundItem[]> {
    const lostCollection = collection(this.firestore, 'lost_items');
    return from(getDocs(lostCollection)).pipe(
      map(snapshot => {
  ...
  createLostItem(
    ...
    try {
      ...
      formData.append('image', blob, `${cleanName}_lost.png`);
    } catch (e) {
    ...
    return this.http.post<any>(`${this.apiBaseUrl}/lost-item`, formData).pipe(
      switchMap(res => {
      ...
      catchError(err => {
        console.warn('FastAPI backend post /lost-item failed or offline. Saving to Firestore directly.', err);
  ...
  private saveToFirestore(item: LostFoundItem): Observable<LostFoundItem> {
    const docRef = doc(this.firestore, 'lost_items', item.id);
    const firestoreData = {
      id: item.id,
  ...
  deleteLostItem(id: string): Observable<boolean> {
    // Delete from Firestore
    const docRef = doc(this.firestore, 'lost_items', id);
    return from(deleteDoc(docRef)).pipe(
      map(() => true),
      catchError(err => {
        console.error('Firestore delete lost item failed:', err);
        return of(false);
      })
    );
  }
